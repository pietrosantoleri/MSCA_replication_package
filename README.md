...
Script [06](scripts/06_export_submitted_tables.do) inserts the estimates from scripts 03 and 04 into the submitted table layouts stored under `scripts/`. Full-precision estimates are available in the [table CSV](output/source_data/table_estimates.csv) and [figure CSV](output/source_data/figure_estimates.csv). Script headers identify individual table columns and figure panels. Supplementary analyses are not included.

## Data and methods

Script 01 prepares analysis variables from the generated records, including synthetic CV-adjusted affiliation indicators, country codes and citation inputs. The package starts from these application-level records; administrative-data preparation, author matching and bibliometric linkage are not reproduced.

Table notes describe the paper's empirical measures, represented here by synthetic substitutes. Successful execution checks the workflow on artificial data.
...
This package folder is the repository root. The original research project and confidential datasets are kept outside the repository.
